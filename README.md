...
- Sidebar file Explorer that mirrors the vault's folder tree, synced
  ...
  collapse/expand folders, `Enter` to zoom into the node's neighborhood,
  `o` to open in Neovim. Picking a file highlights its node in the graph;
  conversely, `hjkl`-navigating the graph moves the Explorer cursor to
  the matching file row and auto-expands any collapsed ancestor folders
  along the way
