...
CHANGELOG-driven, fully automatic. Move `## [Unreleased]` items into a dated `## [X.Y.Z] - YYYY-MM-DD` section, open a PR, merge to `main`. `.github/workflows/release.yml` fires on `pull_request: [closed]` filtered to `paths: CHANGELOG.md`, parses the topmost dated heading, runs the lint/compile gate, then tags `vX.Y.Z` and publishes the GitHub release using the CHANGELOG body. Idempotent — checks tag-exists and release-exists separately, so a partial failure can be resumed via `workflow_dispatch` and only the missing side is re-created. A `concurrency: release-main` group prevents manual dispatch from racing the PR-merge run. Pre-release suffixes (e.g. `0.2.0-rc.1`) are detected automatically.
